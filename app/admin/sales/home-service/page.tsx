...
export default async function HomeServiceSalesPage({ searchParams }: { searchParams: Promise<{ from?: string; to?: string }> }) {
  ...
  const ensure = (rawName: string) => {
    ...
    if (!totals.has(name)) totals.set(name, { name, count: 0, totalAmount: 0 });
  ...
  const rows = Array.from(totals.values())
    .map((t) => ({ ...t, companyShare: t.totalAmount * COMPANY_SHARE, technicianShare: t.totalAmount * TECHNICIAN_SHARE }))
    .sort((a, b) => {
  ...
  const grandTotal = rows.reduce(
    ...
    { count: 0, totalAmount: 0, companyShare: 0, technicianShare: 0 }
  );
          ...
          <tbody>
            ...
            {rows.map((r) => (
              <tr key={r.name} className={`border-b border-slate-200 last:border-0 ${r.name === "Unassigned" ? "opacity-60" : ""}`}>
                ...
                <td className="py-3 pr-3 font-semibold text-slate-900">{peso(r.totalAmount)}</td>
            ...
            {rows.length > 0 && (
              <tr className="font-semibold text-slate-900">
                ...
                <td className="pt-3 pr-3">{peso(grandTotal.totalAmount)}</td>
