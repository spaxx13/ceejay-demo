...
export default async function TechnicianEarningsDetailPage({
  ...
  const [user, technicians, allRepairRecords, allAgreements] = await Promise.all([
    getCurrentUser(),
    getTechnicians(),
    getRepairRecords(),
  ...
  const jobs = computeTechnicianEarnings(technician.name, repairRecords, agreements, from, to);
