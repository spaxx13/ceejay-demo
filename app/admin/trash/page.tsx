...
import {
  ...
  permanentlyDeleteHomeServiceRequest,
  restoreRepairRecord,
          ...
          <div className="space-y-3 sm:hidden">
            ...
            {deletedWalkIns.map((r) => (
              <div key={r.id} className="card space-y-2">
                ...
                <div className="flex gap-1.5 pt-1">
                  <RestoreButton id={r.id} action={restoreWalkInRequest} className="btn-secondary flex-1 !py-1.5 text-xs" />
                  <DeleteButton
                    id={r.id}
              ...
              <tbody>
                ...
                {deletedWalkIns.map((r) => (
                  <tr key={r.id} className="border-b border-slate-200 last:border-0">
                    ...
                    <td className="py-3">
                      <div className="flex gap-1.5">
                        <RestoreButton id={r.id} action={restoreWalkInRequest} />
                        <DeleteButton
                          id={r.id}
                          action={permanentlyDeleteWalkInRequest}
                          confirmMessage={`Permanently delete walk-in registration ${r.reference}? This can't be undone.`}
